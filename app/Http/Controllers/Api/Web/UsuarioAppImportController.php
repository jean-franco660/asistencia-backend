    ...
    public function stats(Request $request)
    {
        ...
        try {
            ...
            $ultimaImportacion = ImportacionLog::tipo(ImportacionLog::TIPO_USUARIOS_APP)
                ...
                ->latest('created_at')
                ->first()
                ?? ImportacionLog::tipo(ImportacionLog::TIPO_USUARIOS_APP)
                    ->latest('created_at')
                    ->first();
            ...
            if ($ultimaImportacion) {
                ...
                // Usar completado_en cuando exista, o iniciado_en si sigue en curso
                $fechaCompleta = $ultimaImportacion->completado_en ?? $ultimaImportacion->iniciado_en;
                $fecha = $fechaCompleta ? $fechaCompleta->format('Y-m-d') : null;
                $hora = $fechaCompleta ? $fechaCompleta->format('H:i:s') : null;

                $ultimaImportData = [
                    'id' => (int) $ultimaImportacion->id,
